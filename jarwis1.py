...
def callback(recognizer, audio):
    try:
        speech_as_text = recognizer.recognize_google(audio)#, keyword_entries=keywords) #Uses Sphinx to recognise speech
...
def start_recognizer(_time = 5): #initial keyword call
    ...
    time.sleep(_time) #keeps loop running
...
#Greetings---------------------------------------------------------------------
        ...
        if data in hello_list:
            hour=time.datetime.datetime.now().hour
